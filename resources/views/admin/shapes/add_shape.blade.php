	...
	<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa fa-table fa-fw "></i> 
				Shapes and Lines 
			<span>&nbsp;>&nbsp;@yield('sectionTitle', "Add Shape or Line")</span>
		</h1>
		<a href="{{url('/')}}/admin/shapes" class="h5"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
	</div>
...
<div class="col-xs-12 col-sm-12 col-md-8 col-lg-6 col-lg-push-3 col-md-push-2 single-box-page">
	<div class="well no-padding">
		<form id="@yield('formId','addShapeForm')" class="smart-form client-form" novalidate="novalidate">
			{{csrf_field()}}
			@yield('requestType')
			@if(isset($shape))
				<input id="shape-id" type="hidden" value="{{$shape->id}}" name="shape_id">
			@endif
			<header>
				@yield('formTitle', 'Add New Shape or Line')
			</header>

			<fieldset>
				<section class="" style="display: inline-block; width: 100%">
					<label class="label col-sm-4 col-xs-6">Shape / Line Image(s)<span class="text-danger bold" style="padding-left: 2px"><strong>*</strong></span></label>
				...
				<section class="" style="display: inline-block; width: 100%">
					...
					<label class="toggle custom-toggle input col-sm-8 col-xs-7">
						@if (isset($shape))
							<input type="checkbox" name="active" @if ($shape->active == 1) {{"checked"}} @endif>
						@else
					...
					@if(isset($shape))
						<div id="edit-preview" class="dz-preview dz-file-preview" style="width: 20%">
							<div class="dz-details">
								<div class="dz-image">
									<img data-dz-thumbnail class="custom-file-preview"  src="{{url('/').Storage::url($shape->image_path)}}" style="width: 100%" />
								</div>
...
<div class="modal fade" id="shape-added" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
				<h4 class="modal-title" id="myModalLabel">@yield('successModalTitle', 'Item Added')</h4>
			</div>
			...
			<div class="modal-footer">
				...
				@endif
				<a href="{{url('/')}}/admin/shapes" class="btn btn-sm btn-default">Shapes list</a>
				
			</div>
		</div>
	</div>
</div> 
<div class="modal fade" id="shape-add-error" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
...
<div class="modal fade" id="overwriteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button id="overwriteConfirmed" type="button" data-dismiss="modal" class="btn btn-sm btn-primary">Yes</button>
				<button type="button" data-dismiss="modal" class="btn btn-sm btn-default">No</button>
